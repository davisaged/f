-- Coloque este LocalScript em StarterPlayerScripts e rode Play/Play Solo.
-- Restaura o menu com tabs (design anterior), mantém FOV circular visível (mesmo com menu fechado)
-- e integra o ESP atualizado (caixa do tamanho do personagem, nome em cima, barra de vida vertical com %,
-- arma embaixo e distância abaixo da arma).
...
-- sizes & theme (keeps design you had)
...
-- FOV circle visuals (always visible independent of menu)
...
-- defaults (attributes used by menu)
...
-- update loop: FOV visuals + ESP update + apply some client-only attributes
RunService.RenderStepped:Connect(function()
	...
	for model,data in pairs(espStore) do
		if not data.bill or not data.bill.Parent then espStore[model] = nil else
			...
			-- compute top & bottom world points to approximate pixel height
...
print("[SinglePlayerMenu] menu restored with ESP & FOV. Open with Insert/M or botão.")
