...
local titleLabel = new("TextLabel", {Parent = titleBar, Text = "MENU", Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, TextColor3 = theme.text, Font = Enum.Font.GothamBold, TextSize = 18})
...
local sidebar = new("Frame", {Parent = mainFrame, Size = UDim2.new(0,160,1,0), Position = UDim2.new(0,0,0,40), BackgroundColor3 = theme.side})
new("UICorner", {Parent = sidebar, CornerRadius = UDim.new(0,8)})
local content = new("Frame", {Parent = mainFrame, Position = UDim2.new(0,170,0,50), Size = UDim2.new(1,-180,1,-60), BackgroundColor3 = theme.panel})
...
local function makeTabBtn(name, icon)
	local btn = new("TextButton", {
		...
		Position = UDim2.new(0,8,0,8 + (#sidebar:GetChildren()-1)*50),
...
do
	...
	local espBox = makeToggle(p, "ESP BOX", true, 36, function(v) screenGui:SetAttribute("ESP_BOX", v) end)
	local espName = makeToggle(p, "ESP NAME", true, 74, function(v) screenGui:SetAttribute("ESP_NAME", v) end)
	local espHealth = makeToggle(p, "ESP HEALTH", true, 112, function(v) screenGui:SetAttribute("ESP_HEALTH", v) end)
	local espDist = makeToggle(p, "ESP DISTANCE", true, 150, function(v) screenGui:SetAttribute("ESP_DISTANCE", v) end)
	local espHighlight = makeToggle(p, "ESP HIGHLIGHT", false, 188, function(v) screenGui:SetAttribute("ESP_HIGHLIGHT", v) end)
	makeLabel(p, "Marker color", 228)
	local colors = {Color3.fromRGB(255,255,255), theme.accent, Color3.fromRGB(255,100,100), Color3.fromRGB(100,200,100)}
	for i,c in ipairs(colors) do
		local b = new("TextButton", {Parent = p, Size = UDim2.new(0,32,0,24), Position = UDim2.new(0, 12 + (i-1)*40, 0, 260), BackgroundColor3 = c, Text = ""})
...
-- Mira tab (visual-only)
do
	local p = pages.Mira.frame
	makeLabel(p, "Mira (visual only)", 6)
	local fovSlider = makeSlider(p, "FOV Size (px)", 40, 500, 150, 36, function(v) screenGui:SetAttribute("FOV_SIZE", v) end)
	...
	local showFOV = makeToggle(p, "Show FOV Circle", true, 170, function(v) screenGui:SetAttribute("SHOW_FOV", v) end)
end
...
do
	...
	local fovLocal = makeSlider(p, "Local Camera FOV", 50, 120, camera.FieldOfView, 96, function(v) camera.FieldOfView = v end)
	local showPos = makeToggle(p, "Show Position", false, 156, function(v) screenGui:SetAttribute("SHOW_POS", v) end)
...
do
	...
	makeLabel(p, "World", 6)
	makeLabel(p, "This tab contains client-side UI settings only.", 36)
	local bgTrans = makeSlider(p, "Menu Background Transparency", 0, 100, 0, 76, function(v) mainFrame.BackgroundTransparency = v/100 end)
	local accentSize = makeSlider(p, "Accent Thickness (UIStroke)", 1, 6, 2, 132, function(v)
...
do
	...
	local infoLabel = new("TextLabel", {Parent = p, Text = "Weapon info shows client-side only.", Size = UDim2.new(1,-12,0,40), Position = UDim2.new(0,6,0,64), BackgroundTransparency = 1, TextColor3 = theme.subtext, Font = Enum.Font.Gotham, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left})
	RunService.RenderStepped:Connect(function()
		local char = LocalPlayer.Character
...
-- ESP manager (for models tagged "TestTarget")
local espStore = {}
...
RunService.RenderStepped:Connect(function()
	...
	for model,data in pairs(espStore) do
		if not data.gui or not data.gui.Parent then espStore[model] = nil else
			...
			-- highlight if inside FOV visual (purely visual)
			...
			-- apply color accent to frame stroke
